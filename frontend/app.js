...
const MODULES=[
 {id:'command', n:1, label:'Command Center', stub:false},
 {id:'demand', n:2, label:'Demand Planning', stub:false},
 {id:'replen', n:3, label:'Replenishment', stub:false},
 {id:'ats', n:4, label:'Vendor ATS', stub:false},
 {id:'deals', n:5, label:'Vendor Deals', stub:false},
 {id:'match', n:6, label:'PO & 3-Way', stub:false},
 {id:'pricing', n:7, label:'Pricing', stub:false},
 {id:'markdown', n:8, label:'Markdown', stub:false},
 {id:'comms', n:9, label:'Team Comms', stub:false},
 {id:'admin', n:10, label:'Data Health', stub:false},
];
const loaded={}; const charts={};
function nav(){
  const el=document.getElementById('nav');
  el.innerHTML=MODULES.map(m=>`<button data-m="${m.id}" ${m.id==='command'?'class="active"':''}><span class="n">${m.n}</span>${m.label}${m.stub?'<span class="stub">SPEC</span>':''}</button>`).join('');
  el.querySelectorAll('button').forEach(b=>b.onclick=()=>show(b.dataset.m));
}
function header(m,title,sub){
  return `<div class="crumb">{ MODULE ${m.n} OF 10 · ${m.stub?'DESIGN SPEC':'LIVE — DATABRICKS'} }</div><h1>${title}</h1><div class="subtitle">${sub}</div>`;
}
function show(id){
  document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.m===id));
...
function build(){
  ...
  main.innerHTML=MODULES.map(m=>`<div class="module" id="mod-${m.id}" style="${m.id==='command'?'':'display:none'}">${SHELLS[m.id](m)}</div>`).join('');
}
function stub(items){
  return `<div class="stubgrid">`+items.map(i=>`<div class="card"><span class="phase">${i[2]}</span><h3>${i[0]}</h3><div class="hint" style="margin-top:6px">${i[1]}</div></div>`).join('')+`</div>`;
}
const SHELLS={
command:m=>header(m,'Command Center','Value-driver KPIs across the network — stockouts, turns, margin, reconciliation. All figures live from sset1000.supplychain.')+`
  <div class="kpis autoload" id="kpirow"><div class="loading">{ LOADING FROM DATABRICKS }</div></div>
  <div class="row2">
    <div class="card focus"><h3>Weekly sales &amp; rain-day contribution</h3><div class="hint">Trailing 12 months · POS net sales with rain-event days split out.</div><div class="chartbox"><canvas id="ch-trend"></canvas></div></div>
    <div class="card"><h3>Weekly margin dollars</h3><div class="hint">Gross margin with promo share.</div><div class="chartbox"><canvas id="ch-margin"></canvas></div></div>
  </div>`,
  ...
  <div class="row2 mt">
    ...
    <div class="card"><h3>Price file ingestion — by vendor tier</h3><div class="hint">How price updates reach staging. Nothing touches the ERP without an approval click.</div><div id="pr-sources"><div class="loading">{ LOADING }</div></div>
      <ul class="feat" style="margin-top:10px">
        ...
        <li><b>Execution trail:</b> approved events auto-spawn retag/label tasks per store and an LLM-drafted change digest (Module 9).</li>
...
comms:m=>header(m,'Team Communications & Store Tasks','LLM-drafted, human-approved comms; store checklists with overdue escalation.')+`
  <div class="kpis autoload" id="c-kpis"></div>
  <div class="card focus" id="c-tbl"><div class="loading">{ LOADING }</div></div>`,
admin:m=>header(m,'Admin & Data Health','Feed freshness, crossref completeness, manual-table stewardship — housekeeping made visible.')+`
...
async command(){
  ...
  const K=[
   ...
   ['Inventory turns', kpi.inventory_turns, 'guardrail: watch vs in-stock', ''],
   ['Gross margin', kpi.margin_pct+'%', 'trailing 12 months', ''],
   ['Sales — 30 days', fmt$(kpi.sales_30d), 'est lost to stockouts: '+fmt$(kpi.est_lost_sales_30d), ''],
   ['3-way auto-clear', kpi.auto_clear_pct+'%', kpi.open_exceptions+' open · '+fmt$(kpi.open_exception_amt), Number(kpi.auto_clear_pct)>=90?'good':'warn'],
   ['Replen queue', kpi.replen_suggestions, 'suggestions awaiting review', ''],
   ['Open deal savings', fmt$(kpi.open_deal_savings), kpi.open_deals+' vendor offers open', 'good'],
   ['Store tasks', kpi.open_tasks, kpi.overdue_tasks+' overdue', Number(kpi.overdue_tasks)>0?'warn':'good'],
  ];
  document.getElementById('kpirow').innerHTML=K.map(k=>`<div class="kpi ${k[3]}"><div class="lbl">${k[0]}</div><div class="val">${k[1]}</div><div class="sub">${k[2]}</div></div>`).join('');
  ...
  mkChart('ch-trend','line',{labels,datasets:[
    {label:'Net sales',data:tr.map(r=>+r.net_sales),borderColor:'#2E5BFF',backgroundColor:'rgba(46,91,255,.08)',fill:true,tension:.3,pointRadius:0,borderWidth:2},
    {label:'Rain-day sales',data:tr.map(r=>+r.rain_day_sales),borderColor:'#1B9E9E',backgroundColor:'rgba(27,158,158,.15)',fill:true,tension:.3,pointRadius:0,borderWidth:1.5}]},
    {scales:{y:{ticks:{callback:v=>'$'+(v/1000)+'k'}}}});
  mkChart('ch-margin','bar',{labels,datasets:[
...
async admin(){
  ...
  document.getElementById('a-tbl').innerHTML='<h3>Table registry & feed freshness</h3><div class="hint">Orange = manually managed/seeded (the housekeeping work stream from the call).</div>'+table(rows,[
